...
if (isset($_SESSION['user'])) {
    ...
    try {
        ...
        // Calculate the number of distinct shops in the cart
        $shopCountStmt = $conn->prepare("
            SELECT COUNT(DISTINCT admin_id) AS shop_count
            FROM cart c
            LEFT JOIN products p ON p.id = c.product_id
            WHERE c.user_id = :user
        ");
        $shopCountStmt->execute(['user' => $user['id']]);
        $shopCount = $shopCountStmt->fetch()['shop_count'];

        // Calculate shipping cost as 100 per shop
        $shipping = $shopCount * 100;
        ...
        if (empty($output)) {
            ...
            $output .= "
                <tr>
                    <td colspan='8' align='right'><b>Shipping</b></td>
                    <td>&#8369; " . number_format($shipping, 2) . "</td>
                </tr>
                <tr>
                    <td colspan='8' align='right'><b>Total</b></td>
                    <td><b>&#8369; " . number_format($total + $shipping, 2) . "</b></td>
                </tr>
            ";
        }
